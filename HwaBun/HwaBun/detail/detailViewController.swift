...
class detailViewController: UIViewController {
    ...
    func updateUI() {
        ...
        if let potInfo = viewModel.potInfo {
            humidLabel.text = "\(potInfo.humidInfo)%"
            tempLabel.text = "\(potInfo.tempInfo)℃"
            soilHumidLabel.text = "\(potInfo.soilHumidInfo)%"
            
            if potInfo.isWatering == true {
